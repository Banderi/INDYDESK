...
func generate_tileset():
	var sprites_list = $ScrollContainer/VBoxContainer
	for n in sprites_list.get_children():
		n.free()
	
	var tile_set = $Floor.tile_set
	tile_set.clear()
	...
	for i in SECTIONS.TILE.size():
		...
		var tile_data = SECTIONS.TILE[i]
		var tile_flags = tile_data.flags
#		var tile_flags = get_applied_flags(tile_data.flags,TileFlags)
...
#		if "is_machete" in tile_flags && "is_weapon" in tile_flags:
#			pass
#		var debug_break = false
#		for flag in tile_flags:
#			match flag:
#				"is_floor","has_transparency","is_object","is_draggable","is_roof":
#					pass
#				"is_empty","is_item","is_weapon","is_town","is_unsolved_puzzle","is_solved_puzzle","is_unsolved_travel","is_character":
#					pass
#				_:
#					debug_break = true
#		if debug_break && i > 500:
#			pass
		...
		match character.type:
			...
			CharacterType.weapon:
				...
				if character.sprites2[0] != -1:
					...
					sprite_add_4way_anim($HERO.attack_spritesheet, c_name, character.sprites2, [2,10,2], [4,12,4], [5,13,5], [7,15,7])
...
func load_indy():
	load_daw("E:/Games/INDYDESK/DESKTOP.DAW")
	generate_tileset()
	generate_spritesheets()
	$Sprite.texture = texture_from_data(SECTIONS["STUP"], 288, 288, PALETTE_INDY)
func load_zone(id):
...
#			var tile = tile_layers.x
#			if tile == 65535:
#				var a = 2
			tiles_1.set_cell(x, y, -1 if tile_layers.x == 65535 else tile_layers.x)
			tiles_2.set_cell(x, y, -1 if tile_layers.y == 65535 else tile_layers.y)
			tiles_3.set_cell(x, y, -1 if tile_layers.z == 65535 else tile_layers.z)
	
	$Label.text = "Type: <%d> %s" % [zone_data.type,Log.get_enum_string(ZonesYoda, zone_data.type)]
	$Label.text += "\n\nHotspots:"
	for hotspot in SECTIONS.HTSP.get(id,[]):
		$Label.text += "\n(%d,%d) <%d> %s %s" % [
			hotspot.x, hotspot.y,
			hotspot.type, Log.get_enum_string(HotspotsYoda, hotspot.type),
			"" if hotspot.args == 65535 else str("(",hotspot.args,")")
		]
		tiles_htsp.set_cell(hotspot.x,hotspot.y,0)
	
	$Label.text += "\n\nMonsters:"
	for monster in SECTIONS.ZAUX[id].monsters:
		$Label.text += "\n(%d,%d) <%d> %s" % [
			monster.x, monster.y,
	...
	$Label.text += "\n\nRequired items:"
	for item in SECTIONS.ZAUX[id].items:
		$Label.text += "\n<%d> %s" % [
			item,
